...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_specific_kwargs
        ...
        if self.config.is_encoder_decoder:
            ...
            ## print('in generate, encoder_outputs:', type(encoder_outputs), len(encoder_outputs), encoder_outputs[0].shape)
        ...
        if self.config.is_encoder_decoder:
            ...
            cur_len = 1
            ## print('changed input_ids:', input_ids.shape, decoder_start_token_id)
    ...
    def _generate_no_beam_search(
        ...
        '''
        if past is not None:
            print('past:', type(past), len(past), type(past[0]), len(past[0]), past[0][0].shape)
        '''
        ...
        while cur_len < max_length:
            ...
            ## print('cur_len: ', cur_len, 'decoder_input_ids shape:', model_inputs['decoder_input_ids'].shape, 'decoder_input_ids:', model_inputs['decoder_input_ids'])
            '''
            print('in generation_utils, model_inputs:', type(model_inputs), type(model_inputs))
            ...
            '''
            print('in generation_utils, outputs:', len(outputs))
            for tid, temp in enumerate(outputs):
                if type(temp) == torch.Tensor:
                    print('generation_utils:', tid, temp.shape)
            '''
            next_token_logits = outputs[0][:, -1, :]

            output_logits.append(outputs[0])
            ## print('in generate, outputs shape:', outputs[0].shape)
        ...
        ## print('in generate, after while, outputs shape:', outputs[0].shape)
