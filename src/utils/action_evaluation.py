...
IGNORE_ATTRIBUTES_LOWER = [e.lower() for e in IGNORE_ATTRIBUTES]
    ...
    for model_datum in model_actions:
        ...
        for round_id, round_datum in enumerate(model_datum["predictions"]):
            ...
            if supervision is None:
                continue
            # Make it uncased, TODO: check compatibility
            supervision = {k.lower(): (v.lower() if isinstance(v, str) else \
                                           ([vv.lower() for vv in v] if isinstance(v, list) else v))
                           for k, v in supervision.items()}
            # Case 1: Action mismatch -- record False for all attributes.
            if not action_match:
                for key in supervision.keys():
                    # Fix ignore attributes
                    if key in IGNORE_ATTRIBUTES_LOWER:
                        continue
            ...
            # Case 2: Action matches -- use model predictions for attributes.
                ...
                for key in supervision.keys():
                    ...
                    if key not in round_datum["attributes"]:
                        matches["attributes"].append(False)
                        continue
